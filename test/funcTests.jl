...
facts("Goal construction") do
  ...
  context("assp") do
    res = assp(x->vars_equal(x, [1]), list(Pair([1], "b"), Pair([0], "a")))
    @fact res --> Pair([1], "b")
    res = assp(x ->vars_equal(x, [3]), list(Pair([4], 6), Pair([3], 5), Pair([2], 6), Pair([1], 5)) )
    @fact res --> Pair([3], 5)
  end

  empty_state=Pair(nil(), 0)
  call_empty_state(g) = g(empty_state)
  some_state = Pair(list(Pair([0], 6)), 1)
  some_state2 = Pair(list(Pair([0], 5)), 1)
  five = x -> equals(x, 5)
  ...
  context("call/fresh") do
    @fact call_fresh(five)(empty_state) --> list(Pair(list(Pair([0], 5)), 1))
    @fact call_fresh(five)(some_state) --> list(Pair(list(Pair([1], 5), Pair([0], 6)), 2))
    @fact call_fresh(five)(some_state2) --> list(Pair(list(Pair([1], 5), Pair([0], 5)), 2))
    #println(take(1, call_fresh(q-> fives(q))(empty_state)))
    @fact take(1, call_fresh(q-> fives(q))(empty_state)) --> list(Pair(list(Pair([0], 5)), 1))
    @fact cdr(take(2, call_fresh(x -> fives(x))(empty_state))) --> list(Pair(list(Pair([0], 5)), 1))
    #fives = x -> disj(equals(x, 5), s_c ->() -> ((fives(x))(s_c)))
  ...
  context("a-and-b") do
    ...
    @fact car(a_and_b_result) --> Pair(list(Pair([1], 5), Pair([0], 7)), 2)
    @fact take(1, a_and_b_result) --> list(Pair(list(Pair([1], 5), Pair([0], 7)), 2))
    @fact car(cdr(a_and_b_result)) --> Pair(list(Pair([1], 6), Pair([0], 7)), 2)
    @fact cdr(cdr(a_and_b_result)) --> nil()

    @fact take(2, a_and_b(empty_state)) --> list(
      Pair(list(Pair([1], 5), Pair([0], 7)), 2),
      Pair(list(Pair([1], 6), Pair([0], 7)), 2))

    @fact take_all(a_and_b(empty_state)) --> list(
      Pair(list(Pair([1], 5), Pair([0], 7)), 2),
      Pair(list(Pair([1], 6), Pair([0], 7)), 2))
  end
  ...
  context("conj and disj") do
    ...
    @fact exp1(empty_state) --> list(Pair(list(Pair([0], 4), Pair([1], 3)), 2))
    ...
    @fact exp3(empty_state) --> list(Pair(list(Pair([0], 3)), 1), Pair(list(Pair([0], 4)), 1))#TODO: fix
  end
...
facts("Function calls") do
  ...
  context("ground_appendo") do
    @fact car(ground_appendo(empty_state)()) --> Pair(list(list([2], "b"), Pair([1], nil()), Pair([0], "a")), 3)
  end

  context("ground_appendo2") do
    @fact car(ground_appendo2(empty_state)()) -->
      Pair(list(list([2], "b"), list([1]), Pair([0], "a")), 3)
  end

  context("appendo") do
    @fact take(2, call_appendo(empty_state)) --> list(
        Pair(list(list([0], [1], [2], [3]), Pair([2], [3]), list([1])), 4),
        Pair(list(list([0], [1], [2], [3]), Pair([2], [6]), list([5]),
          cons([3], cons([4], [6])), cons([1], cons([4], [5]))), 7)
    )
  end

  context("appendo2") do
    @fact take(2, call_appendo2(empty_state)) --> list(
      Pair(list(list([0], [1], [2], [3]), Pair([2], [3]), list([1])), 4),
      Pair(list(list([0], [1], [2], [3]), cons([3], cons([4], [6])),
        Pair([2], [6]), list([5]), cons([1], cons([4], [5]))), 7)
    )
  end
  ...
  @fact string(map(reify_state_1st_var, res)) --> string(list(
    list(nil(), "_.0", "_.0"),
  ...
  context("many_non_ans") do
    ...
    @fact take(1, many_non_ans(empty_state)) --> list(Pair(list(Pair([0], 3)), 1))
  end
